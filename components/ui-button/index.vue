<template>
	<view :style="{
		...
		<button class="ui-button" :disabled="disabled" :data-param="dataParam" :hover-stop-propagation="true" :lang="lang"
...
<script>
// https://ext.dcloud.net.cn/plugin?id=2527
	export default {
		computed: {
			loadingColor() {
		...
		methods: {
			...
			getElQuery() {
				return new Promise(resolve => {
					...
					//#endif
					queryInfo.select('.ui-button').boundingClientRect()
					queryInfo.exec(data => {
						resolve(data)
					})
				})
